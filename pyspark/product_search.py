...
query = sqlparse.format(open('/mnt/c/etl_project/queries/product_search.sql', 'r'))

df_transaction.createOrReplaceTempView('search')
results = spark.sql(f'{query}')
results.show(20)
results.toPandas().to_csv('/mnt/c/etl_project/output/pyspark/product_search.csv', index=False)
