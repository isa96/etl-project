...
query = sqlparse.format(open('/mnt/c/etl_project/queries/product_transaction.sql', 'r'))

df_transaction.createOrReplaceTempView('transaction')
results = spark.sql(f'{query}')
results.show(20)
results.toPandas().to_csv('/mnt/c/etl_project/output/pyspark/product_transaction.csv', index=False)
